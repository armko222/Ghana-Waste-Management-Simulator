...
class ReportingServiceTest {
    ...
    void reportOnEmptyDb() {
        ...
        assertTrue(report.contains("OPERATIONAL REPORT"), "Report should contain the header");
        assertTrue(report.contains("Requests by Status"), "Report should contain status section");
        assertTrue(report.contains("Requests by Category"), "Report should contain category section");
        assertTrue(report.contains("Requests by Priority"), "Report should contain priority section");
        assertTrue(report.contains("Resource Utilisation"), "Report should contain utilisation section");
        assertTrue(report.contains("Deadline Comparison"), "Report should contain deadline comparison section");
    }
    ...
    void reportShowsNoCompletedRequests() {
        String report = service.generateOperationalReport();
        assertTrue(report.contains("no completed requests yet"),
    ...
    void reportWithSeededData() {
        // Seed a service request
        try (var stmt = db.connect().createStatement()) {
            stmt.execute("INSERT INTO locations (location_id, name, area, location_type, x_coord, y_coord) "
                    + "VALUES ('L001', 'Test Location', 'Area1', 'House', 5.0, 0.0)");
            stmt.execute("INSERT INTO locations (location_id, name, area, location_type, x_coord, y_coord) "
                    + "VALUES ('L002', 'Test Location 2', 'Area1', 'School', 5.1, 0.1)");
        ...
        String report = service.generateOperationalReport();
        assertTrue(report.contains("NEW"), "Report should show NEW status count");
    ...
    void reportWithCompletedRequest() {
        try (var stmt = db.connect().createStatement()) {
            stmt.execute("INSERT INTO locations (location_id, name, area, location_type, x_coord, y_coord) "
                    + "VALUES ('L001', 'Loc A', 'Area1', 'House', 5.0, 0.0)");
            stmt.execute("INSERT INTO locations (location_id, name, area, location_type, x_coord, y_coord) "
                    + "VALUES ('L002', 'Loc B', 'Area2', 'Office', 5.1, 0.1)");
            stmt.execute("INSERT INTO service_requests "
                    + "(request_id, source_location_id, destination_location_id, category, "
                    + "urgency, priority, time_submitted, deadline, status) "
                    + "VALUES ('Q100', 'L001', 'L002', 'Hazardous', 5, 'High', "
        ...
        String report = service.generateOperationalReport();
        assertTrue(report.contains("Q100"), "Report should include the completed request");
        assertTrue(report.contains("ON TIME") || report.contains("OVERDUE"),
